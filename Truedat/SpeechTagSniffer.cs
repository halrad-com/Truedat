...
namespace Truedat
{
    ...
    /// marker actually asserts. These are speech evidence — the tags a podcast
    /// app writes are the strongest file-level hint that content is talk, not
    /// music. Three different claims used to be flattened into one
    /// "first marker wins" verdict:
    ...
    /// </summary>
    internal sealed class SpeechMarkers
    {
    ...
    /// <summary>
    /// Detects explicit speech markers embedded in the audio FILE itself —
    /// the tags a podcast downloader writes even when the library XML carries no
    ...
    /// </summary>
    internal static class SpeechTagSniffer
    {
        ...
        /// <summary>Every speech marker found in the file, or null when none were.</summary>
        public static SpeechMarkers? TryDetectAll(string path)
        {
            try
            {
        ...
        internal static SpeechMarkers? TryDetectAllCore(Stream fs)
        {
            try
            {
                var markers = new SpeechMarkers();
        ...
        private static void ScanId3v2(Stream fs, byte[] head, SpeechMarkers markers)
        {
            int major = head[3];
        ...
        private static void ScanMp4(Stream fs, SpeechMarkers markers)
        {
        ...
        private static void WalkAtoms(Stream fs, long end, string[] descend, int depth, ref int visited, SpeechMarkers markers)
        {
            var hdr = new byte[8];
