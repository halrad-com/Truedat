...
Each worker runs Essentia + `ComputeFileMd5` + `ComputeFingerprintV1` + `ComputeAudioStreamSha256FromFile` + `ComputeBitUsage` (Phase 2.5) concurrently via `Task.Run` + `Task.WaitAll`. Wall-clock is ~`max(analysis, slowest-task)` per track. All managed hashes are pure-managed (no subprocess), no path-escape exposure. The `audioStreamSha256` task hashes the TagLib invariant audio region with SHA-256 — content-stable, portable, fast (SHA-NI hardware accel). `ComputeBitUsage` is the one subprocess-bearing task (ffmpeg pipe → s32le walk) — silently returns null when ffmpeg.exe is absent, so the worker still completes.
