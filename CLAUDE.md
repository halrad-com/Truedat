...
Each worker runs Essentia + `ComputeFileMd5` + `ComputeFingerprintV1` + `ComputeAudioStreamSha256FromFile` + `ComputeBitUsage` (Phase 2.5) + `ComputeHfEnergyRatio` (Phase 3) concurrently via `Task.Run` + `Task.WaitAll`. Wall-clock is ~`max(analysis, slowest-task)` per track. All managed hashes are pure-managed (no subprocess), no path-escape exposure. The `audioStreamSha256` task hashes the TagLib invariant audio region with SHA-256 — content-stable, portable, fast (SHA-NI hardware accel). `ComputeBitUsage` and `ComputeHfEnergyRatio` are the subprocess-bearing tasks (ffmpeg pipes) — both silently return null when ffmpeg.exe is absent, so the worker still completes. `ComputeHfEnergyRatio` itself spawns two sub-ffmpegs (total RMS + highpass-filtered RMS) that run concurrently inside the outer task.
