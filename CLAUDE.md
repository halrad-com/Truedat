...
Tiers 2/3/4 and the cache-miss worker fan-out all route body reads through one lazily-opened staged copy via `EnsureStagedSrc()` — opened on first body read, reused for every subsequent read in the tier walk and the workers, disposed in `finally`. So a tier-2/3/4 hit on a UNC track copies once and amortizes that copy across the tier's hash + the worker fan-out, instead of re-reading the same source N times across the network.
