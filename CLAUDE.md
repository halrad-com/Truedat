...
- **Harmonic-texture wave (2026-08-09, v0.5.4.7, nullable, omit-when-missing)**: `hpcp12` (12-semitone chroma vector — the extractor's 36-bin `tonal.hpcp.mean` folded 3→1 per semitone by `FoldHpcp36To12`, normalized to unit max, 4dp; **null on a no-chroma/silent track** so absence never fakes an all-zero similarity vector) and `dynamicComplexity` (`lowlevel.dynamic_complexity` scalar, 4dp). **Essentia-derived → NOT backfillable and NOT in the re-extract canary** (same posture as the 2026-07-22 wave): fresh analyses populate them; existing entries stay tier-1 cache hits and lack them until a deliberate re-analysis (`--refresh-features`). **There is no parse-only backfill** — the raw extractor JSON is written to a per-track temp file and deleted at parse in `AnalyzeWithEssentiaCore`'s `finally`, so populating an existing catalog is a normal (byte-reading) re-analysis, not a cheap re-fold. Primary consumer: MBXHub AutoQ's harmonic-texture continuity term (cross-track `hpcp12` cosine) plus a small dynamics term (`dynamicComplexity` delta) — a preference weight beside Camelot, never a gate; the blend is null-safe, so a half-populated catalog behaves exactly as today.
