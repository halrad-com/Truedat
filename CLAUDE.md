...
4. **Cross-SHA** (`moodShaIndex`, `(cached·sha)` / `[CACHED·sha]`) — different path, invariant audio region matches (covers clean moves, copies with preserved mtimes, and moved-plus-tag-edited files alike). Re-keys old → new path; recomputes `fileMd5` + `fingerprint.v1`. (The former tier-3 cross-MD5 was removed 2026-07-11 — cross-SHA catches a strict superset; the "4" name is kept so hit tags and history stay legible.)
