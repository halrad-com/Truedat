...
**Opens the 0.5.5.0 line. No source changes since `v0.5.4.9-RC7` — this is that code
under the new version.** 0.5.5.0 is expected to stand for an extended beta, so the stage
tag rather than the version number is what distinguishes builds during it; EV1 is the
first, and it is public.
